  ...
  attributesValuesToSring(){
    if(this.fillable !== [] && this.fillable.length !== 0){
      if(this.fillable.length == 1){
        return QueryBuilder.prepare(this[this.fillable[0]], this.jsonRows.includes(this.fillable[0]));
      } 
      return this.fillable.reduce((prev, current, i, array)=>{
        let separator = (i > array.length) ? '' : ',';
        return (i === 1) 
          ? `${QueryBuilder.prepare(this[prev], this.jsonRows.includes(prev))}${separator}${QueryBuilder.prepare(this[current], this.jsonRows.includes(current))}` 
          : `${prev}${separator}${QueryBuilder.prepare(this[current],  this.jsonRows.includes(current))}`;
      });
