...
class Nightmare extends QueryBuilder{
  ...
  update(){
    let query = `UPDATE ${this.table} SET (${this.attributes.toString()}) = (${this.attributesValuesToSring()}) WHERE id=${this.id} RETURNING *`;
    return new Promise(resolve => {
      this.pool.query(query)
      .then(res => {
        this.fill(res.rows[0]);
  ...
  delete(rowName = false, rowValue = false){
    if((this.id && typeof this.id !== 'undefined' && this.attributes !== []) || rowName && rowValue){
      let query = (rowName && rowValue) 
        ? `DELETE FROM ${this.table} WHERE ${rowName}=${QueryBuilder.prepare(rowValue)}`
