...
const actionsTypes = {
  SELECT:  'TRUE_FALSE_FALSE_FALSE',
  COUNT: 'FALSE_TRUE_FALSE_FALSE',
  WHERE: 'FALSE_FALSE_TRUE_FALSE',
  DELETE: 'FALSE_FALSE_FALSE_TRUE',
  LIMIT: LIMIT,
  OFFSET: OFFSET,
  SELECT_WHERE: 'TRUE_FALSE_TRUE',
  COUNT_WHERE: 'FALSE_TRUE_TRUE',
  DELETE_WHERE: 'FALSE_FALSE_TRUE_TRUE'
}
...
class QueryBuilder {
  ...
  static prepare(value){
    let scapedValue = QueryBuilder.realScape(value);
    return isNaN(value) ? `'${scapedValue}'` : scapedValue; 
  }
  ...
  queryReducer(){
    ...
    count = this.queryType.includes(COUNT), destroy = this.queryType.includes(DELETE);
    result = `${select}_${count}_${where}_${destroy}`.toUpperCase();
    switch (result) {
